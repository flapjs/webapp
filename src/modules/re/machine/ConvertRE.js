...
export function convertToNFA(reMachine)
{
    return AST2NFA(reMachine.root, reMachine);
}
...
function AST2NFA(astNode, re)
{
    ...
    {
        case KLEENE:
            return kleene(AST2NFA(astNode.children[0], re));
        case PLUS:
            return plus(AST2NFA(astNode.children[0], re));
        case CONCAT:
            return concat(AST2NFA(astNode.children[0], re), AST2NFA(astNode.children[1], re));
        case UNION:
            return or(AST2NFA(astNode.children[0], re), AST2NFA(astNode.children[1], re));
        case '(':
            return AST2NFA(astNode.children[0], re);
        default:
            throw new Error('You\'ve got a weird node in the AST tree with symbol ' + astNode.symbol);
    }
}
